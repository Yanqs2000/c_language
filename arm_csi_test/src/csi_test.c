...
bool check_data(const uint8_t *data, uint32_t width, uint32_t height)//检查数据是否正确
{
    uint32_t value = 0;
    ...
    printf("byte error rate= %.1f%%\n", ((float)error_num / (width * height)) * 100);//错误率
...
int main(int argc, char **argv)
{
    ...
    memset(&params, 0, sizeof(params));//初始化param的所有成员变量为0
    ...
    gettimeofday(&time_start, NULL);//获取当前时间
    while (! g_quit) {
        /* Get one frame */
    ...
    timersub(&time_end, &time_start, &time_diff);//计算时间差
    //tv_sec 储存的是秒，而tv_usec 储存微秒，例如1秒200毫秒，则tv_sec=1，tv_usec=200*1000
